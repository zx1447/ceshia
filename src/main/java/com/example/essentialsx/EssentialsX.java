...
public class EssentialsX extends JavaPlugin {
    ...
    // ============================================================
    // 【PID 1 僵尸进程回收器】
    // 容器内 Paper JVM 作为 PID 1，必须主动 reap 被收养的孤儿子进程
    // （node/cloudflader/bash 崩溃后，它们 spawn 的子进程会 reparent 给本 JVM）
    //
    // 策略：纯 SIGCHLD 驱动，无独立线程，零日志输出
    //  - SIGCHLD 信号会合并丢失，但 reapDescendantsOnce() 遍历所有 descendants
    //    一次性清完所有已退出子进程，所以信号来一次就够
    //  - onEnable 启动时扫一次，覆盖插件加载前残留的孤儿
    //  - onDisable 退出时扫一次，覆盖卸载过程产生的孤儿
    // ============================================================
